    ...
    async def execute_batch(batch: BatchRequest, request: Request):
        ...
        async def run_script(script: str, language: str) -> ScriptResult:
            ...
            async with semaphore:
                try:
                    sandbox = await asyncio.to_thread(Sandbox.new, client=client, ttl_seconds=timeout)
...
def parse_args():
    ...
    if not args.api_key:
        raise ValueError(
            'MorphCloud API key not provided. Please set MORPH_API_KEY environment variable or use --api_key.'
        )
